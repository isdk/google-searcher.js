...
describe('GoogleSearcher Options', () => {
  ...
  it('should map timeRange presets correctly', () => {
    expect(searcher.testFormatOptions({ timeRange: 'hour' }).extraParams).toContain('tbs=qdr%3Ah');
    expect(searcher.testFormatOptions({ timeRange: 'day' }).extraParams).toContain('tbs=qdr%3Ad');
    expect(searcher.testFormatOptions({ timeRange: 'week' }).extraParams).toContain('tbs=qdr%3Aw');
    expect(searcher.testFormatOptions({ timeRange: 'month' }).extraParams).toContain('tbs=qdr%3Am');
    expect(searcher.testFormatOptions({ timeRange: 'year' }).extraParams).toContain('tbs=qdr%3Ay');
  });
  ...
  it('should map custom timeRange correctly', () => {
    ...
    expect(result.extraParams).toContain('tbs=cdr%3A1%2Ccd_min%3A1%2F1%2F2023%2Ccd_max%3A12%2F31%2F2023');
  });
  ...
  it('should map categories correctly', () => {
    expect(searcher.testFormatOptions({ category: 'images' }).extraParams).toContain('tbm=isch');
    expect(searcher.testFormatOptions({ category: 'videos' }).extraParams).toContain('tbm=vid');
    expect(searcher.testFormatOptions({ category: 'news' }).extraParams).toContain('tbm=nws');
  });
  ...
  it('should map safeSearch', () => {
    expect(searcher.testFormatOptions({ safeSearch: 'strict' }).extraParams).toContain('safe=active');
    expect(searcher.testFormatOptions({ safeSearch: 'off' }).extraParams).toContain('safe=images');
  });

  it('should map offset to start param when offset > 0', () => {
    expect(searcher.testFormatOptions({ offset: 10 }).extraParams).toContain('start=10');
    expect(searcher.testFormatOptions({ offset: 20 }).extraParams).toContain('start=20');
  });

  it('should not include start param when offset is 0 or undefined', () => {
    expect(searcher.testFormatOptions({ offset: 0 }).extraParams).not.toContain('start=');
